<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column align="center" label="类型" show-overflow-tooltip min-width="85">
        <template slot-scope="scope">
          <span>{{ scope.row.actionType==1?'查询流量':'查询状态' }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="报错信息" show-overflow-tooltip min-width="90">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="处理状态" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-tag :type="scope.row.status==2?'success':'warning'">{{ scope.row.status==2?'已处理':'未处理' }}</el-tag>
        </template>
      </el-table-column>
    </el-table>
...
import { findQueryException } from '@/api/card'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      findQueryException(this.listQuery, '.table').then(res => {
        if (+res.status === 0) {
