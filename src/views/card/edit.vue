...
export default {
  ...
  methods: {
    ...
    upload(param) {
      ...
      uploadFileFinally('/plat/card/v4/excel/card/batchUpdate', param, this.listQuery).then(res => {
        const data = res.data
        this.$notify({
    ...
    downLoad() {
      window.open('http://wx.szcoolfish.com/upload/excelmodel/updateCard.xlsx', '_self')
    }
  }
}
</script>
