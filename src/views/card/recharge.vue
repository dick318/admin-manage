...
export default {
  ...
  methods: {
    upload(param) {
      ...
      uploadFileFinally('/plat/card/v4/excel/card/batchUpdateCardMoney', param).then(res => {
        const data = res.data
        this.$notify({
    ...
    downLoad() {
      window.open('http://wx.szcoolfish.com/upload/excelmodel/batchUpdateCardMoney.xlsx', '_self')
    }
  }
}
</script>
