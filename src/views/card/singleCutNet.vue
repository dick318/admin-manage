  ...
  <div class="app-container">
    ...
    <el-form ref="form" :inline="true" label-width="9rem" style="margin-top:15px;vertical-align: bottom;" >
      <el-form-item label="操作类型">
        <el-select v-model="type" class="filter-item" filterable clearable placeholder="请选择操作类型">
          <el-option
...
export default {
  ...
  methods: {
    upload(param) {
      ...
      uploadFileFinally('/plat/card/v4/excel/batchSingleCutNet', param, { type: this.type }).then(res => {
        const data = res.data
        this.$notify({
    ...
    downLoad() {
      window.open('http://wx.szcoolfish.com/upload/excelmodel/singleCutNet.xlsx', '_self')
    }
  }
}
</script>
