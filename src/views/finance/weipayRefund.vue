  ...
  <div class="app-container">
    ...
    <el-form ref="form" :inline="true" label-width="9rem" style="margin-top:15px;vertical-align: bottom;" >
      <el-form-item label="密码">
        <el-input v-model.trim="password" class="filter-item" style="width:220px" clearable placeholder="请输入密码"/>
      </el-form-item>
...
export default {
  ...
  methods: {
    upload(param) {
      ...
      uploadFileFinally('/plat/finance/v4/weipayBatchRefund', param, { password: this.password }).then(res => {
        const data = res.data
        this.$notify({
        ...
        if (data.status === 0) {
          ...
          this.$router.push('/finance/payList')
        }
        loading.close()
      }).catch(() => {
        loading.close()
    ...
    downLoad() {
      window.open('http://wx.szcoolfish.com/upload/excel/model/refund.xlsx', '_self')
    }
  }
}
</script>
