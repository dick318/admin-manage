<template>
  <div class="app-container">
    ...
    <el-button v-permission="['kuyuplat:packageBatch:update']" class ="uploadFile" style="margin-left: 10px;" size="small" type="success" @click="submitUpload">上传到服务器</el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    upload(param) {
      ...
      uploadFileFinally('/plat/business/v4/updateBatchFlowPackages', param).then(res => {
        const data = res.data
        this.$notify({
    ...
    downLoad() {
      window.open('http://wx.szcoolfish.com/upload/excelmodel/updatePackage.xlsx', '_self')
    }
  }
}
</script>
