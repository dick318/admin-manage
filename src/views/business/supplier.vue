<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-permission="['kuyuplat:operator:save']" v-waves class="filter-item" size="small" type="warning " icon="el-icon-plus" @click="handleAdd" />
      <el-button v-permission="['kuyuplat:account:search']" v-waves class="filter-item" size="small" type="info" @click="jump('/business/accounts')">帐号</el-button>
      <!-- <el-button v-waves class="filter-item" size="small" type="info" @click="jump('/business/rules')">规则</el-button> -->
    </div>
    ...
    <el-table
      ...
      <el-table-column align="center" label="添加时间" show-overflow-tooltip min-width="180">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.actions')" align="left" show-overflow-tooltip min-width="250px">
        <template slot-scope="scope">
          <!-- <el-button v-permission="['common']" type="info" size="mini" @click="AddRules(scope.row)">规则</el-button> -->
          <el-button v-permission="['kuyuplat:operator:update']" type="primary" size="mini" icon="el-icon-edit-outline" @click="handleUpdate(scope.row)"/>
          <el-button v-permission="['kuyuplat:operator:delete']" type="danger" size="mini" icon="el-icon-delete" @click="handleDelete(scope.row.id)"/>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" :rules="rules" :inline="true" label-width="9rem" class="dialog">
        <el-form-item v-if="dialogStatus=='update'||dialogStatus=='create'" label="自定义名称:" prop="name">
          <el-input :disabled="dialogStatus=='update'" v-model.trim="temp.name" clearable />
        </el-form-item>
        <el-form-item v-if="dialogStatus=='update'||dialogStatus=='create'" label="运营商类型:" prop="operatorType">
        ...
        <el-form-item v-if="dialogStatus=='update'||dialogStatus=='create'" label="套餐分类:" prop="packageType">
        ...
        <el-form-item v-if="dialogStatus=='rules'" label="套餐规则:">
          <el-select v-model="temp.packageType" class="filter-item" clearable placeholder="请选择套餐规则">
            <el-option v-for="item in packageTypeSelect" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="dialogStatus=='rules'" label="限速规则:">
          <el-select v-model="temp.packageType" class="filter-item" clearable placeholder="请选择限速规则">
            <el-option v-for="item in packageTypeSelect" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item
...
export default {
  ...
  methods: {
    ...
    AddRules(row) {
      this.temp = {
        id: row.id,
        rulePackage: row.rulePackage,
        ruleSpeed: row.ruleSpeed
      }
      this.dialogStatus = 'rules'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
      })
        ...
        if (valid) {
          ...
          if (this.dialogStatus === 'create') {
            saveOperator(this.temp, '.el-dialog__footer')
              .then(res => {
                if (+res.status === 0) {
                  store.dispatch('setOperatorStatus', false)
                  this.$notify({
                    title: '结果',
                    message: res.message,
                    duration: 2000,
