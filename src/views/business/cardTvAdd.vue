...
import { worklistSearch } from '../guide/defineSteps'
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.guide) {
      this.driver.defineSteps(worklistSearch)
      this.driver.start()
    }
  },
  methods: {
    upload(param) {
      ...
      uploadFileFinally('/plat/card/v4/excel/cardTv/batchAdd', param).then(res => {
        const data = res.data
        this.$notify({
        ...
        if (data.status === 0) {
          ...
          this.$router.push('/business/cardTv')
        }
        loading.close()
      }).catch(() => {
        loading.close()
    ...
    downLoad() {
      window.open('http://wx.szcoolfish.com/upload/excelmodel/cardTvbatchAdd.xlsx', '_self')
    }
  }
}
</script>
