<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-permission="['kuyuplat:problem:add']" v-waves class="filter-item" size="small" type="primary" icon="el-icon-plus" @click="addProblem" />
    </div>
    <el-table
      :data="list"
      :fit ="true"
      ...
      <el-table-column align="center" label="排序" show-overflow-tooltip min-width="165">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.actions')" align="left" show-overflow-tooltip min-width="200px">
        <template slot-scope="scope">
          <el-button v-permission="['kuyuplat:problem:update']" type="primary" size="mini" icon="el-icon-edit-outline" @click="update(scope.row)"/>
          <el-button v-permission="['kuyuplat:problem:delete']" type="danger" size="mini" icon="el-icon-delete" @click="handleDelete(scope.row.id)"/>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" :rules="rules" :inline="true" label-position="right" class="dialog" label-width="9rem">
        ...
        <el-form-item label="排序:" prop="sort">
          <el-input v-model.trim="temp.sort"/>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          addProblem(this.temp, '.el-dialog__footer')
            .then(res => {
              if (+res.status === 0) {
                this.$notify({
                  title: '结果',
    ...
    updateData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          updateProblem(tempData, '.el-dialog__footer').then(res => {
            if (+res.status === 0) {
              this.$notify({
                title: '结果',
                message: res.message,
    ...
    handleDelete(id) {
      this.$confirm('此操作将删除该问题列表, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteProblem({ id }, '.el-message-box').then((res) => {
          this.$notify({
    ...
    getList(type) {
      searchProblems(this.listQuery, '.table').then(res => {
        if (+res.status === 0) {
