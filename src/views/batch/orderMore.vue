<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column show-overflow-tooltip align="center" label="查询卡号" min-width="160">
        <template slot-scope="scope">
      ...
      <el-table-column show-overflow-tooltip align="center" label="套餐批次" min-width="120">
        <template slot-scope="scope">
          <span>{{ oidObject[scope.row.oid] }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column show-overflow-tooltip align="center" label="套餐生效类型" min-width="150">
        <template slot-scope="scope">
          <span>{{ packageEffectMap[scope.row.type] }}</span>
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip align="center" label="套餐名称" min-width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.pname }}</span>
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip align="center" label="支付类型" min-width="150">
        <template slot-scope="scope">
          <span style="color:#66b1ff;cursor:pointer">{{ tradeMap[scope.row.tradeType] }}</span>
        </template>
      </el-table-column>
...
import { statusMap, agentArr, accountsArr, operatorArr, packageEffectMap, tradeMap } from '@/utils/mapArr'
      ...
      if (this.list.length === 0) {
       ...
       import('@/vendor/Export2Excel').then(excel => {
         const tHeader = ['id', '查询卡号', '订单号', 'iccid', '接入号', '虚拟号',
           '订单时间', '代理商', '昵称', '套餐批次', '供应商账号', '状态', '套餐生效类型',
           '套餐名称', '支付类型', '订单金额(元)', '结果', '消息']
         const filterVal = [
           'id',
           'cardNo',
           'orderID',
         ...
         excel.export_json_to_excel({
           header: tHeader,
           data,
           filename: `套餐订单批量查询表格${new Date().toLocaleDateString()}`,
    ...
    action() {
      ...
      if (cardNoArr.length > 0) {
        for (const [index, elems] of cardNoArr.entries()) {
          ...
          setTimeout(() => {
            searchOrders({ cardNo: elem }).then(res => {
              if (res.data.rows.length > 0) {
                res.data.rows.map((v, i) => {
                  const row = Object.assign(v, { statusResult: res.status, message: res.message })
                  for (var item = 0; item < this.list.length; item++) {
