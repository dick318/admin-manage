  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form label-width="45px">
        ...
        <el-form-item label="原因">
          <el-input v-model.trim="actionReason" class="filter-item" style="width:200px" clearable placeholder="操作原因"/>
          <el-button v-waves size="small" type="primary" class="filter-item " @click="action(8)" >强复</el-button>
          <el-button v-waves size="small" type="danger" class="filter-item " @click="action(7)" >强停</el-button>
    ...
    <el-table
      ...
      <el-table-column show-overflow-tooltip align="center" label="操作类型" min-width="150">
        <template slot-scope="scope">
          <el-tag :type="scope.row.type==8?'success':'warning'">{{ scope.row.type|typeFilter }}</el-tag>
        </template>
      </el-table-column>
...
import { superChangeCardStatus } from '@/api/cardInfo'
export default {
    ...
    action(type) {
      ...
      if (msisdnArr.length > 0) {
        for (const [index, elems] of msisdnArr.entries()) {
          ...
          setTimeout(() => {
            superChangeCardStatus({ msisdn: elem, type, actionReason: this.actionReason }).then(res => {
